...
impl<'a> Game<'a> {
    pub (crate) fn load_content(&mut self, ttf_context: &Sdl2TtfContext, texture_creator: &'a TextureCreator<WindowContext>) {
        ...
        let title_textured_content = TexturedContent {
            ...
            texture: texture,
            src: None,
            dst: Some(rect!(SCREEN_WIDTH/2 - width/6, 50, width/3, height/3))
        };
        ...
        let right_bar_outside = DrawnContent {
            ...
            rect: rect!(SCREEN_WIDTH-105, 75, 5, 600),
            color: Color::RGB(50, 50, 255)
        };
        ...
        let right_bar_inside = DrawnContent {
            ...
            displayed_in_pause: false,
            name: Some(String::from_str("right_bar").unwrap()),
            rect: rect!(SCREEN_WIDTH-104, 76, 3, 598),
            color: Color::RGB(0, 0, 0)
        };
        ...
        self.drawn.push(DrawnContent {
            ...
            rect: rect!(SCREEN_WIDTH/2 - 16, 3*SCREEN_HEIGHT/4 + 50, 32, 32),
            color: Color::RGB(255, 255, 255)
        });
        self.drawn.push(DrawnContent {
            displayed_in_game: true,
            displayed_in_pause: false,
            name: Some(String::from_str("player_weapon").unwrap()),
            rect: rect!(SCREEN_WIDTH/2 - 15, 3*SCREEN_HEIGHT/4 + 51, 30, 30),
            color: Color::RGB(0, 0, 0)
        });
        self.drawn.push(DrawnContent {
            displayed_in_game: true,
            displayed_in_pause: false,
            name: Some(String::from_str("player_weapon").unwrap()),
            rect: rect!(SCREEN_WIDTH/2 - 14, 3*SCREEN_HEIGHT/4 + 52, 28, 28),
            color: Color::RGB(255, 255, 255)
        });
        self.drawn.push(DrawnContent {
            displayed_in_game: true,
            displayed_in_pause: false,
            name: Some(String::from_str("player_weapon").unwrap()),
            rect: rect!(SCREEN_WIDTH/2 - 13, 3*SCREEN_HEIGHT/4 + 53, 26, 26),
            color: Color::RGB(0, 0, 0)
        });
        self.drawn.push(DrawnContent {
            displayed_in_game: true,
            displayed_in_pause: false,
            name: Some(String::from_str("player_weapon").unwrap()),
            rect: rect!(SCREEN_WIDTH/2 - 12, 3*SCREEN_HEIGHT/4 + 54, 24, 24),
            color: Color::RGB(255, 255, 255)
        });
    }
